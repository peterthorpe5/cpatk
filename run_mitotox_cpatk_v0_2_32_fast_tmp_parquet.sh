...
# CPATK v0.2.32 fast mitotox Cell Painting stress-test workflow.
#
...
PROJECT_OUT_DIR="${PROJECT_OUT_DIR:-${OUT_DIR:-${BASE_DIR}/cpatk_v0_2_32_mitotox_fast_${RUN_TAG}}}"
...
for optional_command in cpatk-drift-qc cpatk-neighbours cpatk-moa cpatk-ml cpatk-explain cpatk-latent; do
...
if [[ "${RUN_CLIPN}" == "1" ]] && command -v cpatk-latent >/dev/null 2>&1; then
  ...
  run_soft_step "${OUT_DIR}/12_clipn/.clipn.done" \
    cpatk-latent \
      ...
      --experiment mitotox_cpatk_v0_2_32_fast \
      --mode integrate_all \
...
echo "Primary report on project filesystem: ${PROJECT_OUT_DIR}/CPATK_mitotox_v0_2_32_fast_full_report.html"
