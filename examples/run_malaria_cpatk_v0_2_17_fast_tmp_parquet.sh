...
# CPATK v0.2.17 fast malaria Cell Painting test workflow.
#
...
PROJECT_OUT_DIR="${PROJECT_OUT_DIR:-${OUT_DIR:-${BASE_DIR}/cpatk_v0_2_17_malaria_fast_${RUN_TAG}}}"
...
CLIPN_ZERO_EPSILON="${CLIPN_ZERO_EPSILON:-1e-8}"
...
#   CONDA_ENV_NAME=cpatk qsub run_malaria_cpatk_v0_2_17_fast_full_test.sh
...
#   CPATK_SOURCE_DIR=/path/to/cpatk_v0_2_17_fast_full qsub run_malaria_cpatk_v0_2_17_fast_full_test.sh
...
require_command() {
  ...
  if ! command -v "${command_name}" >/dev/null 2>&1; then
    ...
    echo "Install/activate CPATK v0.2.17, or set CPATK_SOURCE_DIR to the source folder." >&2
    exit 1
  fi
}
...
clipn_zero_epsilon	${CLIPN_ZERO_EPSILON}
...
if [[ "${RUN_CLIPN}" == "1" ]]; then
  ...
  run_soft_step "${OUT_DIR}/12_clipn/.clipn.done" \
    cpatk-clipn \
      ...
      --experiment malaria_cpatk_v0_2_17_fast \
      --mode integrate_all \
...
echo "Primary report on project filesystem: ${PROJECT_OUT_DIR}/CPATK_malaria_v0_2_17_fast_full_report.html"
