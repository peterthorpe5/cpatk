...
# CPATK v0.2.14 end-to-end malaria Cell Painting test workflow.
#
...
OUT_DIR="${OUT_DIR:-${BASE_DIR}/cpatk_v0_2_14_malaria_test_${RUN_TAG}}"
...
# Method toggles. Keep everything on for the first full validation.
...
#   CONDA_ENV_NAME=cpatk qsub run_malaria_cpatk_v0_2_12_full_test.sh
...
#   CPATK_SOURCE_DIR=/path/to/cpatk_v0_2_12_full qsub run_malaria_cpatk_v0_2_12_full_test.sh
...
require_command() {
  ...
  if ! command -v "${command_name}" >/dev/null 2>&1; then
    ...
    echo "Install/activate CPATK v0.2.14, or set CPATK_SOURCE_DIR to the source folder." >&2
    exit 1
  fi
}
...
run_preprocess_strategy() {
  ...
  local extra_args=( ${extra_args_string} )
...
if [[ "${RUN_PREPROCESSING}" == "1" ]]; then
  ...
  run_preprocess_strategy \
    ...
    "--reference_normalisation_method none --batch_correction_method none"

  run_preprocess_strategy \
    "dmso_robust_z" \
    "--reference_normalisation_method robust_z --reference_column ${REFERENCE_COLUMN} --reference_values ${REFERENCE_VALUES} --reference_group_columns Metadata_Plate --batch_correction_method none"

  run_preprocess_strategy \
    "dmso_robust_z_combat_location_scale" \
    "--reference_normalisation_method robust_z --reference_column ${REFERENCE_COLUMN} --reference_values ${REFERENCE_VALUES} --reference_group_columns Metadata_Plate --batch_correction_method combat_location_scale --batch_column ${BATCH_COLUMN} --batch_protect_columns ${BATCH_PROTECT_COLUMNS}"
fi
...
if [[ "${RUN_CLIPN}" == "1" ]]; then
  ...
  run_soft_step "${OUT_DIR}/12_clipn/.clipn.done" \
    cpatk-clipn \
      ...
      --experiment malaria_cpatk_v0_2_12 \
      --mode integrate_all \
...
echo "Primary report: ${OUT_DIR}/CPATK_malaria_v0_2_12_full_report.html"
