...
# CPATK v0.2.16 fast malaria Cell Painting test workflow.
#
...
PROJECT_OUT_DIR="${PROJECT_OUT_DIR:-${OUT_DIR:-${BASE_DIR}/cpatk_v0_2_16_malaria_fast_${RUN_TAG}}}"
...
#   CONDA_ENV_NAME=cpatk qsub run_malaria_cpatk_v0_2_16_fast_full_test.sh
...
#   CPATK_SOURCE_DIR=/path/to/cpatk_v0_2_16_fast_full qsub run_malaria_cpatk_v0_2_16_fast_full_test.sh
...
require_command() {
  ...
  if ! command -v "${command_name}" >/dev/null 2>&1; then
    ...
    echo "Install/activate CPATK v0.2.16, or set CPATK_SOURCE_DIR to the source folder." >&2
    exit 1
  fi
}
...
if [[ "${RUN_CLIPN}" == "1" ]]; then
  ...
  run_soft_step "${OUT_DIR}/12_clipn/.clipn.done" \
    cpatk-clipn \
      ...
      --experiment malaria_cpatk_v0_2_16_fast \
      --mode integrate_all \
...
echo "Primary report on project filesystem: ${PROJECT_OUT_DIR}/CPATK_malaria_v0_2_16_fast_full_report.html"
