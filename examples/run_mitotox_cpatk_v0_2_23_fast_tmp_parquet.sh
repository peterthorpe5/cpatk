...
# CPATK v0.2.23 fast mitotox Cell Painting stress-test workflow.
#
...
PROJECT_OUT_DIR="${PROJECT_OUT_DIR:-${OUT_DIR:-${BASE_DIR}/cpatk_v0_2_23_mitotox_fast_${RUN_TAG}}}"
...
if [[ "${RUN_CLIPN}" == "1" ]] && command -v cpatk-clipn >/dev/null 2>&1; then
  ...
  run_soft_step "${OUT_DIR}/12_clipn/.clipn.done" \
    cpatk-clipn \
      ...
      --experiment mitotox_cpatk_v0_2_23_fast \
      --mode integrate_all \
...
echo "Primary report on project filesystem: ${PROJECT_OUT_DIR}/CPATK_mitotox_v0_2_23_fast_full_report.html"
